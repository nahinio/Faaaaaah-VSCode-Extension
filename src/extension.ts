...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider('faaaaaah.judgeView', provider, {
...
class FaaaaaahViewProvider implements vscode.WebviewViewProvider {
  private _view?: vscode.WebviewView;
  private readonly _ctx: vscode.ExtensionContext;

  constructor(ctx: vscode.ExtensionContext) { this._ctx = ctx; }
  ...
  private _refresh() {
    if (!this._view) { return; }
    const wavUri = this._view.webview.asWebviewUri(
      vscode.Uri.joinPath(this._ctx.extensionUri, 'media', 'Faaaaaah.wav')
    );
    const saved: TestCase[] = this._ctx.workspaceState.get(this._key(), []);
...
async function runTest(filePath: string, test: TestCase): Promise<{ pass: boolean; actual: string; error?: string }> {
...
function execute(cmd: string, stdin: string, cwd: string): Promise<string> {
...
function buildHtml(webview: vscode.Webview, wavUri: string, savedTests: TestCase[]): string {
